      ...
      <div className="modal-content">
        ...
        <div className="flex flex-col w-full text-center">
          ...
          <p className="mt-4 text-3xl mb-7 md:text-lg md:mt-0">
            Please click/tap the appropriate link to help you in your navigation
            of our services
          </p>
        </div>
          ...
          <div className="modal-items-container gap-y-2 md:gap-y-1">
            {ModalLinks.map((modalLink, index) => (
              <a
                key={index}
                href={modalLink.link}
                className="modal-links-container w-full bg-red-800 py-2.5 px-4 h-20 md:h-11 rounded-sm hover:bg-red-900 flex-wrap content-center hover:pl-7 flex space-x-2 text-white"
              >
                <p className="text-4xl text-white md:text-lg font-base">
                  <i className={`mr-3 fa ${modalLink.d}`}></i>
                  {modalLink.title}
                </p>
              </a>
            ))}
